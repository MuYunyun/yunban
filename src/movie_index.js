...
$(function () {
  ...
  var movieIndexFun = (function () {
    ...
    var gallerySwitch = (function () {
      var $oPanel = $('#scrollMovies .panel'),                // 获取顶部轮播图面板对象
        $oTitle = $('#headerNow span'),                     // 获取即将上映标题对象
        page = 1,                                           // 初始页码
        $oLeft = $('#scrollMovies .slide-prev'),            // 获取左箭头按钮
        $oRight = $('#scrollMovies .slide-next'),           // 获取右箭头按钮
        $oThumbnail = $('#scrollMovies .thumbnail'),        // 获取电影对象
      ...
      if (windowWidth < 768) {
        ...
        var marginWidth = $oThumbnail.outerWidth(true) - $oThumbnail.outerWidth();  // 获取海报的外边距
        var oThumbnailWidth = (oCol6_width - marginWidth * 3) / 3;  // 计算每张海报应赋予的宽度值
      } else {   // 否则上映电影区域每个版面展示4张电影
      ...
      $('#scrollMovies .side-max').html(pageTotal);           // 设置两个电影展示区总页数
      $('#screenBody').width(oCol6_width * pageTotal);        // 设置电影展示区总宽度
      $oThumbnail.width(oThumbnailWidth);                     // 设置每张轮播海报图片的宽度
      ...
      $oTitle.on('click', function () {
        ...
        funAjax(URL, 'GET', function (results) {
          ...
          $('#headerNow a').attr('href', '/movie/results?q=' + data.name + '&p=0').text(data.name);   //设置标题名称
          if (galleryName === '即将上映') {         //切换副标题名称
            $oTitle.text('正在热映');
          } else {
            $oTitle.text('即将上映');
          }
          ...
          if (dataLength < $oThumbnail.length) {
            $('#scrollMovies .thumbnail:gt(' + (dataLength - 1) + ')').remove();
          } else if (dataLength > $oThumbnail.length) {  //如果切换后电影列表数量大于原电影数量,则创建多出的节点
            for (var j = $oThumbnail.length; j < dataLength; j++) {
              $('#screenBody').append('<div class="thumbnail scroll-item"><a href=""><img src="" alt=""/></a><div class="caption"><h5></h5><p><a href="" role="button" class="btn btn-primary">观看预告片</a></p></div></div>');
            }
            // 给新添加的电影节点设置宽度
            $('#screenBody .thumbnail:gt(' + ($oThumbnail.length - 1) + ')').width(oThumbnailWidth);
          }
          $oThumbnail = $('#scrollMovies .thumbnail');     //重新获取电影对象
          len = $oThumbnail.length;       // 重新获取轮播展示区电影数量及总页数

          windowWidth = $(window).width();                    // 获取当前视口宽度
          // 如果当前视口宽度小于768，则上映电影区域每个版面展示3张电影
          if (windowWidth < 768) {
            pageTotal = Math.ceil(len / 3);                   // 即将电影轮播展示区总页数
            marginWidth = $oThumbnail.outerWidth(true) - $oThumbnail.outerWidth();  // 获取海报的外边距
            oThumbnailWidth = (oCol6_width - marginWidth * 3) / 3;  // 计算每张海报应赋予的宽度值
          } else {   // 否则上映电影区域每个版面展示4张电影
          ...
          if (page > pageTotal) {
            page = pageTotal;
            $('#scrollMovies .side-index').html(page);
      ...
      function funMoving(direction) {
        ...
        if (!$('#screenBody').is(':animated')) {
          if (direction === 'right') {          //向右移动
            if (page === pageTotal) {           //在最右边继续往右滚回到最左边
              page = 1;
              // 设置显示当前电影页码
              $('#scrollMovies .side-index').html(page);
              ...
              page++;
              $('#scrollMovies .side-index').html(page);  //设置显示当前电影页码
              // 找到单击元素所在电影滚动面板元素
              $('#screenBody').animate({ left: '-=' + oCol6_width }, 500); //逻辑有点绕
            }
          } else {   //向左移动
      ...
      $('#scrollMovies').on('mouseover', function () {
        clearInterval(timer);
    ...
    $('#fliterMovies .class-top').on('click', 'button', function () {
      ...
      if ($(this).is('.btn-primary')) {
        ...
        $('#fliterMovies input[checked]').removeAttr('checked');
        $('#fliterMovies input[value=pv]').attr('checked', 'checked');  //html增加checked
        $('#fliterMovies input[value=pv]').prop('checked', 'checked');  //当切换类别时,排序默认显示为按热度排序(页面上显示效果)
          ...
          if (results.data === null || results.data.movies.length === 0) {
            ...
            for (var j = 0; j < oThumbnail.length; j++) {
              ...
              if (data[j]) {
                ...
                $(oThumbnail[j]).find('h5').html(data[j].title + '&nbsp;<stong style="color: #e09015;">' + data[j].rating);
                ...
                if (data[j].poster) {
                  if (data[j].poster.indexOf('http') > -1) {
                    ...
                    $oImg.attr('src', '/upload/movie/' + data[j].poster).attr('alt', data[j].title);
                  }
                }
              }
            }
    ...
    /*加载更多部分*/
    var count = 0;
    $('#fliterMovies .more').on('click', function () {
      var catName = $('#fliterMovies .btn-primary').html();       // 获取按钮文字内容
      var checkedName = $('#fliterMovies input[checked]').val();    //获取当前排序种类名
      count++;
      var URL = '/movieIndex?catName=' + encodeURIComponent(catName + '电影') + '&p=' + count + '&checkedName=' + checkedName;   //对中文进行编码
      //发送Ajax请求
      funAjax(URL, 'GET', function (results) {
        var data = results || [];               // 获取Ajax返回的电影数据
        if (data.length && data.length > 0) {
          for (var i = 0; i < data.length; i++) {
            if (data[i].poster.indexOf('http') > -1) {
              $('#classBody').append('<div class="col-md-3 col-sm-3 col-xs-4"><div class="thumbnail"><a href="/movie/' + data[i]._id + '"><img src="' + data[i].poster + '" alt="' + data[i].title + '"/></a><div class="caption"><h5>' + data[i].title + '&nbsp;<stong style="color: #e09015;">' + data[i].rating + '</stong></h5></div></div></div>');
            } else {        //自行上传图片
              $('#classBody').append('<div class="col-md-3 col-sm-3 col-xs-4"><div class="thumbnail"><a href="/movie/' + data[i]._id + '"><img src="/upload/movie/' + data[i].poster + '" alt="' + data[i].title + '"/></a><div class="caption"><h5>' + data[i].title + '&nbsp;<stong style="color: #e09015;">' + data[i].rating + '</stong></h5></div></div></div>');
            }
          }
        } else {           //无更多结果
          $('#fliterMovies .more').html('无更多结果');
        }
      });
      $('#fliterMovies .btn-default').on('click', function () {    //当切换类别时,count重新置0
        count = 0;
        $('#fliterMovies .more').html('加载更多');  //每次点击类别时重置为加载更多
      });
      $('#fliterMovies input[name="sort"]').on('click', function () {    //当切换排序种类时,count重新置0
        count = 0;
      });
    });

    /*选电影选排序种类*/
    $('#fliterMovies input[name="sort"]').on('click', function () {
      $('#fliterMovies .more').html('加载更多');  //每次点击排序时重置为加载更多
      var sortName = $(this).val();
      $('#fliterMovies input[checked]').removeAttr('checked');
      $('#fliterMovies input[value=' + sortName + ']').attr('checked', 'checked');
      $('#fliterMovies input[value=' + sortName + ']').prop('checked', 'checked');   //把相应的checked属性移过去
      var catName = $('#fliterMovies .btn-primary').html();       // 获取按钮文字内容
      var URL = '/movieIndex?sortName=' + sortName + '&category=' + encodeURIComponent(catName + '电影');   //对中文进行编码
      //发送Ajax请求
      funAjax(URL, 'GET', function (results) {
        var data = results || [];               // 获取Ajax返回的电影数据
        if (data.length && data.length > 0) {
          $('#fliterMovies .col-md-3').remove();
          for (var i = 0; i < data.length; i++) {
            if (data[i].poster.indexOf('http') > -1) {
              $('#classBody').append('<div class="col-md-3 col-sm-3 col-xs-4"><div class="thumbnail"><a href="/movie/' + data[i]._id + '"><img src="' + data[i].poster + '" alt="' + data[i].title + '"/></a><div class="caption"><h5>' + data[i].title + '&nbsp;<stong style="color: #e09015;">' + data[i].rating + '</stong></h5></div></div></div>');
            } else {        //自行上传图片
    ...
    var galleryFrames = (function () {
      var page = 1,                                            // 页码变量
        $oLeft = $('#galleryFrames .slide-prev'),            // 获取左箭头按钮
        $oRight = $('#galleryFrames .slide-next'),           // 获取右箭头按钮
        len = $('#galleryFrames li').length,                 // 热门推荐轮播图片数量
        $oUl = $('#galleryFrames ul'),                       // 获取轮播图列表的对象
        $oItem = $('#galleryFrames .slide-item');            // 获取展示区对象

      $('#galleryFrames .side-max').html(len);             // 设置轮播图总数量
      $oItem.width(oCol6_width);                           // 设置每张轮播图片Li的宽带
        ...
        if (!$('oUl').is(':animated')) {
          if (direction === 'right') {          //向右移动
            if (page === len) {           //在最右边继续往右滚回到最左边
              page = 1;
              // 设置显示当前电影页码
              $('#galleryFrames .side-index').html(page);
              $oUl.animate({ left: 0 }, 500);
            } else {                            //不在最右边的情况
              page++;
              $('#galleryFrames .side-index').html(page);  //设置显示当前电影页码
              // 找到单击元素所在电影滚动面板元素
    ...
    var cinemasSearch = (function () {
      ...
      $('#citiesList .cities-list-item').on('click', 'a', function () {
        $city.children('span').html($(this).text());       //点击城市名替换默认的宁波
